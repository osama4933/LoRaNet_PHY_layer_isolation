...
def act_and_send(Pkt_To_Send, queue_size):
    ...
    BW_obs = math.log(bandwid / BACKUP_BW) / 3
    ...
    # if pkt_counter > 309 and pkt_counter < 312: # --> TODO: breaks a packet apart here!!!
    #     plt.plot(buffer.real)
    #     plt.show()
...
def sending_queue(server_socket):
    ...
    while True:
        if not out_q.empty():
            ...
            bandwid = BACKUP_BW * math.exp(History[pkt_num].BW_obs * 3)
...
def process_rewards():
    ...
    while True:
        if rew_counter in Rewards:
            ...
            store_results(save_dir + 'Single_rewards.txt', store_vals)
...
if __name__ == "__main__":
    ...
    if SAVE_RL:
       print("saving RL...")                # todo: we need to save this on the fly occasionally
       my_ppo.save()
    time.sleep(1)
